...
namespace shelf_project.Controllers.DistributorManagement
{
    ...
    public class ContractController : Controller
    {
        ...
        /// <param name="systemSettingService">システム設定サービス</param>
        public ContractController(ApplicationDbContext context, UserManager<ApplicationUser> userManager, ISystemSettingService systemSettingService)
        {
            _context = context;
        ...
        public async Task<IActionResult> Index(int? locationId)
        {
            ...
            // システム設定から月額料金を取得
            decimal monthlyFee;
            if (targetDistributor.DistributorType == DistributorType.Individual)
            {
            ...
            else if (targetDistributor.DistributorType == DistributorType.Store)
            {
                monthlyFee = await _systemSettingService.GetSettingValueAsDecimalAsync(
        ...
        private async Task DeleteDistributorAndRelatedData(Distributor distributor)
        {
            ...
            if (distributor.QRCode != null)
            {
                ...
                var qrCodeProducts = await _context.QRCodeProducts
                    .Where(qcp => qcp.QRCodeId == distributor.QRCode.Id)
                    .ToListAsync();
                _context.QRCodeProducts.RemoveRange(qrCodeProducts);
                
                _context.QRCodes.Remove(distributor.QRCode);
            }
